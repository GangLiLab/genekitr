...
genInfo <- function(id,
  ...
  # some id matches many entrez id
  ...
  if(length(tomany_id)>0 & length(tomany_id)<3){
    warning(paste0('Some ID matches many entrez id, like "',tomany_id,'"\n',
                   'If you want to get one-to-one match, please set "simple=TRUE"'))
  }else if (length(tomany_id)>3){
    warning(paste0('Some ID matches many entrez id, like "',paste0(tomany_id[1:3],collapse = ', '),'"...\n',
                   'If you want to get one-to-one match, please set "simple=TRUE"'))
  }
