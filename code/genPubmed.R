...
##' @return a text of excel of gene pubmed paper record.
...
##' @examples
##' \dontrun{
##' genPubmed(id = c("Cyp2c23","Fhit","Gal3st2b","Gbp4"), keywords = 'stem cell', field = 'tiab')
##' }
##' 
genPubmed <- function(id,
  ...
  if(field %in% data$Name){
    message(paste0('Search example: ',
                   paste0(id[1],' [',field,'] AND ',keyword,' [',field,'] ')))
  }else{
  ...
  l=sapply(id, function(x){
    query = paste0(x,' [',field,'] AND ',keyword,' [',field,'] ')
    ...
    if(entrez_id$Count == "0"){
      ...
      pub_df <- suppressMessages(easyPubMed::table_articles_byAuth(pubmed_data = myxml, 
